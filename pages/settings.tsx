        ...
        <div className="grid grid-cols-1 gap-2">
          {supportedLocales.map((l) => (
            <button
              ...
              onClick={() => {
                setLocale(l.code);
                toast.success(`${l.label} activado`);
              }}
              className={`flex items-center justify-between px-3.5 py-2.5 rounded-lg text-sm border transition-colors ${
                locale === l.code
                  ? "bg-primary/10 border-primary/40 text-primary font-medium shadow-sm"
                  : "bg-base-300/40 border-base-300/80 text-base-content/70 hover:text-base-content hover:bg-base-300/80"
              }`}
            >
              <div className="flex items-center gap-2.5">
              ...
              {locale === l.code && <span className="text-xs font-bold text-primary">✓</span>}
            </button>
          ))}
        </div>
      </div>
